...
[ -z "${MC_NAME}" ] && export MC_NAME=golem
[ -z "${KIND_CLUSTER_NAME}" ] && export KIND_CLUSTER_NAME=clusterapi
...
eval $(awk -v mc=$AWS_PROFILE '$1 ~ mc {x=NR+4; next; }(NR<=x){print "export "toupper($1)"="$3;}' ~/.aws/credentials)
...
cat aws.yaml | envsubst > /tmp/kind-${KIND_CLUSTER_NAME}.yaml
...
clusterctl init --infrastructure=aws:v2.5.2
...
# Install crossplane
helm install crossplane --namespace crossplane --create-namespace crossplane-stable/crossplane --set args='{"--debug"}'
...
cat ./controllers.yaml | envsubst | kubectl apply -f -
wait_for_crds 'providerconfigs.aws.upbound.io'

cat ./providerconfig.yaml | envsubst | kubectl apply -f -
kubectl apply -f ./functions.yaml
wait_for_functions function-network-discovery
